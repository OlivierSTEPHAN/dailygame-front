...
const ByScreenshots: React.FC = () => {
  ...
  const [correctAnswersFromServer, setCorrectAnswersFromServer] = useState<Screenshot>();
  ...
  useEffect(() => {
    ...
    if (savedState) {
      ...
      const savedState = localStorage.getItem('gameState');
      if (savedState) {
        const { started, currentIndex, answers, score, updatedAt } = JSON.parse(savedState);
  ...
  useEffect(() => {
    ...
    const gameState = { started, currentIndex, answers, score, updatedAt: currentDate };
    localStorage.setItem('gameState', JSON.stringify(gameState));
  }, [started, currentIndex]);

  const fetchScreenshotsUrl = async () => {
    const response = await fetch('https://srv541447.hstgr.cloud:8443/api/daily-games/screenshots/url');
              ...
              <div className="flex flex-col md:flex-row items-start md:items-center justify-center">
                ...
                <div className="mx-auto my-4 relative flex flex-col w-full md:flex-row md:items-center">
                  ...
                  {showSuggestions && suggestions.length > 0 && (
                    <div ref={suggestionsRef} className="absolute top-full left-0 w-full bg-white border border-gray-300 rounded-t-none rounded-b z-10 max-h-60 overflow-y-auto">
                  ...
                  <button
                    className="mt-4 md:mt-0 md:ml-4 p-2 border bg-green-600 text-white rounded w-full md:w-[30%]"
                    onClick={checkAnswer}
                  >
                    Submit
                  </button>
                ...
                <ul className="grid grid-cols-1 md:grid-cols-2 gap-4 text-start">
                  {correctAnswersFromServer && correctAnswersFromServer.name.map((name, index) => (
                    <li key={index} className={`p-4 ${score[index] === 1 ? 'border border-green-500' : 'border border-red-500'} rounded-lg`}>
                      ...
                      <img src={correctAnswersFromServer.url[index]} alt={`Capture d'écran ${index + 1}`} className="mt-2 w-full" />
                      <p className="mt-2"><strong>Correct answer:</strong> {name}</p>
                      <p className="mt-2"><strong>Your answer:</strong> {answers[index] || 'No answer 😢'}</p>
                    </li>
                  ))}
                </ul>
              </div>
